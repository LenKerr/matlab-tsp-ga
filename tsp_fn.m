%TSP_FN Traveling Salesman Problem (TSP) Farthest Neighbor (FN) Algorithm
%   The Farthest Neighbor algorithm produces different results depending on
%   which city is selected as the starting point. This function determines
%   the Farthest Neighbor routes for multiple starting points and returns
...
%     resultStruct = tsp_fn;
%       -or-
%     resultStruct = tsp_fn(userConfig);
%       -or-
%     [...] = tsp_fn('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
%
function varargout = tsp_fn(varargin)
    ...
    if showProg
        hFig = figure('Name','TSP_FN | Current Solution','Numbertitle','off');
        hAx = gca;
        if showStatus
    ...
    %
    % Find the maximum distance route
    %
    [maxDist,index] = max(distHistory);
    optRoute = pop(index,:);
    ...
    if showResult
        ...
        if showProg
            ...
            title(hAx,sprintf('Total Distance = %1.4f',maxDist));
        end
        
        %
        % Plot the FN results
        %
        figure('Name','TSP_FN | Results','Numbertitle','off');
        subplot(2,2,1);
