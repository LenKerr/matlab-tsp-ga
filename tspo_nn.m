%TSPO_NN Open Traveling Salesman Problem (TSP) Nearest Neighbor (NN) Algorithm
...
%     tspo_nn(userConfig)
%       -or-
%     resultStruct = tspo_nn;
%       -or-
%     resultStruct = tspo_nn(userConfig);
%       -or-
%     [...] = tspo_nn('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
function varargout = tspo_nn(varargin)
    ...
    for p = 1:popSize
        ...
        %
        if showProg
            if (dims > 2), plot3(hAx,xy(thisRte,1),xy(thisRte,2),xy(thisRte,3),'r.-');
            else, plot(hAx,xy(thisRte,1),xy(thisRte,2),'r.-'); end
    ...
    if showResult
        ...
        % Plot the NN results
        %
        figure('Name','TSPO_NN | Results','Numbertitle','off');
        subplot(2,2,1);
