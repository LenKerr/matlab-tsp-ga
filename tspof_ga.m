...
%     resultStruct = tspof_ga(userConfig);
%
% See also: tsp_ga, tsp_nn, tspo_ga, tspofs_ga
%
% Author: Joseph Kirk
...
function varargout = tspof_ga(varargin)
    ...
    if showProg
        hFig = figure('Name','TSPOF_GA | Current Best Solution', ...
    ...
    for iter = 1:numIter
        ...
        %     end
        %
        row = [ones(popSize,1) pop];
        col = [pop N(ones(popSize,1))];
        ind = N*(col-1) + row;
