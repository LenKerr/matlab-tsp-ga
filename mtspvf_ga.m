% MTSPVF_GA Fixed Start/End Variable Multiple Traveling Salesmen Problem (M-TSP) Genetic Algorithm (GA)
...
%   the start location to individual cities and back to the original starting place)
%
% Summary:
...
%     mtspvf_ga
%       -or-
%     mtspvf_ga(userConfig)
%       -or-
%     resultStruct = mtspvf_ga;
%       -or-
%     resultStruct = mtspvf_ga(userConfig);
%       -or-
%     [...] = mtspvf_ga('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
%     resultStruct = mtspvf_ga(userConfig);
%
% See also: tsp_ga, mtsp_ga, mtspf_ga, distmat
%
% Author: Joseph Kirk
% Email: jdkirk630@gmail.com
% Release: 2.0
% Release Date: 05/01/2014
function varargout = mtspvf_ga(varargin)
    ...
    if showProg
        hFig = figure('Name','MTSPVF_GA | Current Best Solution','Numbertitle','off');
        hAx = gca;
        if showStatus
    ...
    rng = [[1 optBreak+1];[optBreak n]]';
    for s = 1:nSalesmen
        optSolution{s} = [1 optRoute(rng(s,1):rng(s,2)) 1];
    end
    ...
    if showResult
        ...
        % Plots
        %
        figure('Name','MTSPVF_GA | Results','Numbertitle','off');
        subplot(2,2,1);
