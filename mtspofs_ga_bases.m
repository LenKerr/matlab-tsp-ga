% MTSPOFS_GA_BASES Open Multiple Traveling Salesmen Problem (M-TSP) Genetic Algorithm (GA)
...
%        different (unassigned) individual cities which they stop at
%
% Input:
...
%     Taken together, these represent the solution [1 5 6 9][2 4 8][3 10 7],
%     which designates the routes for the 3 salesmen as follows:
%         . Salesman 1 travels from city 1 to 5 to 6 to 9
%         . Salesman 2 travels from city 2 to 4 to 8
%         . Salesman 3 travels from city 3 to 10 to 7
%
% Usage:
%     mtspofs_ga_bases
%       -or-
%     mtspofs_ga_bases(userConfig)
%       -or-
%     resultStruct = mtspofs_ga_bases;
%       -or-
%     resultStruct = mtspofs_ga_bases(userConfig);
%       -or-
%     [...] = mtspofs_ga_bases('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
%
function varargout = mtspofs_ga_bases(varargin)
    ...
    if showProg
        hFig = figure('Name','MTSPOFS_GA_BASES | Current Best Solution', ...
    ...
    for iter = 1:numIter
        ...
        %     end
        %
        row = [ones(popSize,1) popRoute(:,1:n-1)];
        col = popRoute;
        for p = 1:popSize
            brk = popBreak(p,:);
            row(p,brk+1) = (2:nSalesmen);
        end
        ind = N*(col-1) + row;
        ...
        if (minDist < globalMin)
            ...
            if showProg
                ...
                %
                for s = 1:nSalesmen
                    rte = [s optRoute(rng(s,1):rng(s,2))];
    ...
    if showResult
        ...
        % Plot the GA results
        %
        figure('Name','MTSPOFS_GA_BASES | Results','Numbertitle','off');
        subplot(2,2,1);
