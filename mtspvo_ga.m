% MTSPVO_GA Open Variable Multiple Traveling Salesmen Problem (M-TSP) Genetic Algorithm (GA)
...
%   each city exactly once without returning to their starting location)
%
% Summary:
...
%     mtspvo_ga
%       -or-
%     mtspvo_ga(userConfig)
%       -or-
%     resultStruct = mtspvo_ga;
%       -or-
%     resultStruct = mtspvo_ga(userConfig);
%       -or-
%     [...] = mtspvo_ga('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
%     resultStruct = mtspvo_ga(userConfig);
%
% See also: tsp_ga, mtsp_ga, mtspo_ga, distmat
%
% Author: Joseph Kirk
% Email: jdkirk630@gmail.com
% Release: 2.0
% Release Date: 05/01/2014
function varargout = mtspvo_ga(varargin)
    ...
    if showProg
        hFig = figure('Name','MTSPVO_GA | Current Best Solution','Numbertitle','off');
        hAx = gca;
        if showStatus
    ...
    rng = [[1 optBreak+1];[optBreak n]]';
    for s = 1:nSalesmen
        optSolution{s} = optRoute(rng(s,1):rng(s,2));
    end
    ...
    if showResult
        ...
        % Plots
        %
        figure('Name','MTSPVO_GA | Results','Numbertitle','off');
        subplot(2,2,1);
