% MTSP_GA_MAX Multiple Traveling Salesmen Problem (M-TSP) Genetic Algorithm (GA)
%   Finds a (near) optimal solution to the M-TSP by setting up a GA to search
%   for the LONGEST route (GREATEST distance needed for the salesmen to travel
...
%     - MAXDIST (scalar float) is the total distance traveled by the salesmen
%
...
%     mtsp_ga_max
%       -or-
%     mtsp_ga_max(userConfig)
%       -or-
%     resultStruct = mtsp_ga_max;
%       -or-
%     resultStruct = mtsp_ga_max(userConfig);
%       -or-
%     [...] = mtsp_ga_max('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
%
function varargout = mtsp_ga_max(varargin)
    ...
    if showProg
        hFig = figure('Name','MTSP_GA_MAX | Current Best Solution','Numbertitle','off');
        hAx = gca;
        if showStatus
    ...
    for iter = 1:numIter
        ...
        if (maxDist > globalMax)
            ...
            if showProg
                ...
                title(hAx,sprintf('Total Distance = %1.4f, Iteration = %d',maxDist,iter));
                hold(hAx,'off');
                drawnow;
            end
        end
    ...
    if showResult
        ...
        % Plots
        %
        figure('Name','MTSP_GA_MAX | Results','Numbertitle','off');
        subplot(2,2,1);
        ...
        for s = 1:nSalesmen
            ...
            title(sprintf('Total Distance = %1.4f',maxDist));
            hold on;
        end
        subplot(2,2,4);
