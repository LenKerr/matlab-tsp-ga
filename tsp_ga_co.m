%TSP_GA_CO Traveling Salesman Problem (TSP) Genetic Algorithm (GA)
...
%     tsp_ga_co
%       -or-
%     tsp_ga_co(userConfig)
%       -or-
%     resultStruct = tsp_ga_co;
%       -or-
%     resultStruct = tsp_ga_co(userConfig);
%       -or-
%     [...] = tsp_ga_co('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
%
function varargout = tsp_ga_co(varargin)
    ...
    if showProg
        hFig = figure('Name','TSP_GA_CO | Current Best Solution','Numbertitle','off');
        hAx = gca;
        if showStatus
    ...
    for iter = 1:numIter
        ...
        %   The top two solutions are then used to create two new solutions.
        %   One of the new solutions is also mutated and passed along to the
        ...
        for p = 4:4:popSize
            ...
            %
            childOne = father;
            childOne(any(m(:,ones(n,1)) == father(ones(len,1),:),1)) = [];
            childOne = [childOne(1:I-1) mother(I:J) childOne(I:n-len)];
            childTwo = mother;
            childTwo(any(f(:,ones(n,1)) == mother(ones(len,1),:),1)) = [];
            childTwo = [childTwo(1:I-1) father(I:J) childTwo(I:n-len)];
    ...
    if showResult
        ...
        % Plot the GA results
        %
        figure('Name','TSP_GA_CO | Results','Numbertitle','off');
        subplot(2,2,1);
