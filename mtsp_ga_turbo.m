% MTSP_GA_TURBO Multiple Traveling Salesmen Problem (M-TSP) Genetic Algorithm (GA)
...
%     mtsp_ga_turbo
%       -or-
%     mtsp_ga_turbo(userConfig)
%       -or-
%     resultStruct = mtsp_ga_turbo;
%       -or-
%     resultStruct = mtsp_ga_turbo(userConfig);
%       -or-
%     [...] = mtsp_ga_turbo('Param1',Value1,'Param2',Value2, ...);
%
% Example:
...
function varargout = mtsp_ga_turbo(varargin)
    ...
    tmpPopRoute = zeros(7,n);
    tmpPopBreak = zeros(7,nBreaks);
    newPopRoute = zeros(popSize,n);
    newPopBreak = zeros(popSize,nBreaks);
    [isStopped,isCancelled] = deal(false);
    if showProg
        hFig = figure('Name','MTSP_GA_TURBO | Current Best Solution','Numbertitle','off');
        hAx = gca;
        if showStatus
    ...
    for iter = 1:numIter
        ...
        if (nSame >= 500)
            for p = 1:popSize
                popRoute(p,:) = randperm(n);
                popBreak(p,:) = rand_breaks();
            end
            localMin = Inf;
        else
            ...
            for p = 7:7:popSize
                ...
                for k = 1:7 % Generate New Solutions
                    ...
                    switch k
                        ...
                        case 4 % Modify Breaks
                            tmpPopBreak(k,:) = rand_breaks();
                        case 5 % Flip, Modify Breaks
                            tmpPopRoute(k,I:J) = tmpPopRoute(k,J:-1:I);
                            tmpPopBreak(k,:) = rand_breaks();
                        case 6 % Swap, Modify Breaks
                            tmpPopRoute(k,[I J]) = tmpPopRoute(k,[J I]);
                            tmpPopBreak(k,:) = rand_breaks();
                        case 7 % Slide, Modify Breaks
                            tmpPopRoute(k,I:J) = tmpPopRoute(k,[I+1:J I]);
                            tmpPopBreak(k,:) = rand_breaks();
                        otherwise % Do Nothing
                    end
                end
                newPopRoute(p-6:p,:) = tmpPopRoute;
                newPopBreak(p-6:p,:) = tmpPopBreak;
            end
            popRoute = newPopRoute;
            popBreak = newPopBreak;
            popRoute(popSize,:) = bestRoute;
            popBreak(popSize,:) = bestBreak;
        end
    ...
    if showResult
        ...
        % Plots
        %
        figure('Name','MTSP_GA_TURBO | Results','Numbertitle','off');
        subplot(2,2,1);
